...
class MoodsTableViewController: UITableViewController, SegueHandler {
    ...
    var managedObjectContext: NSManagedObjectContext!//上下文
    ...
    fileprivate func setupTableView() {
        ...
        tableView.estimatedRowHeight = 100//高度
        let request = Mood.sortedFetchRequest
        request.fetchBatchSize = 20
        request.returnsObjectsAsFaults = false
        let frc = NSFetchedResultsController(fetchRequest: request, managedObjectContext: managedObjectContext, sectionNameKeyPath: nil, cacheName: nil)
        dataSource = TableViewDataSource(tableView: tableView, cellIdentifier: "MoodCell", fetchedResultsController: frc, delegate: self)//实现frc的三个代理
    }
